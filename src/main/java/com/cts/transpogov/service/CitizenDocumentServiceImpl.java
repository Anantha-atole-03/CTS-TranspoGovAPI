...
/**
 * Service implementation for managing citizen documents.
 * Handles the logic for uploading, retrieving, and verifying physical and digital documents.
 */
@Service
...
public class CitizenDocumentServiceImpl implements ICitizenDocumentService {
	...
	/**
	 * Links a file path to a citizen and persists document metadata.
	 * * @param request            DTO containing citizen ID and document type.
	...
	public CitizenDocumentResponse uploadDocument(CitizenDocumentCreateRequest request, String filePath,
		...
		CitizenDocument document = CitizenDocument.builder()
				...
				.verificationStatus(DocumentVerificationStatus.valueOf(verificationStatus))
				.build();
	...
	/**
	 * Updates the verification status of an existing document.
	...
	/**
	 * Retrieves all documents belonging to a specific citizen.
	...
	/**
	 * Utility method to transform a CitizenDocument entity into a CitizenDocumentResponse DTO.
