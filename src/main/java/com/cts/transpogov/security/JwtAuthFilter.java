...
/**
 * Security filter that executes once per request to intercept and validate JWT tokens.
 * It extracts the identity from the token and populates the Spring Security Context
...
public class JwtAuthFilter extends OncePerRequestFilter {
	...
	// Services and repositories required to validate the token and load the principal
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			/* 
			 * 4. Parse the token to get the username (phone).
			 * If the token is tampered with or expired, Jwts.parserBuilder() inside 
			 * authUtils will throw an exception caught by our catch block.
			 */
			String phone = authUtils.getUsername(token);

			// 5. If phone exists and user is not already authenticated in this session context
			if (phone != null && SecurityContextHolder.getContext().getAuthentication() == null) {
				...
				if (userOpt.isPresent()) {
					...
					UsernamePasswordAuthenticationToken auth = new UsernamePasswordAuthenticationToken(
							citizen, null, citizen.getAuthorities());
			...
			/* 
			 * 7. If any error occurs (token expired, invalid signature, etc.),
			 * delegate the exception to the global handler to return a 401 response.
			 */
