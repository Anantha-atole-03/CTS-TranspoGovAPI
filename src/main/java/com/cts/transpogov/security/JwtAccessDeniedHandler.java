 ...
 * This class is triggered when a user is successfully authenticated (logged in)
 * but does not possess the required UserRole/Authority to access a specific endpoint.
 */
@Component
public class JwtAccessDeniedHandler implements AccessDeniedHandler {

	/**
	 * Handles the forbidden access scenario (HTTP 403).
	 * Responds with a structured JSON error message when permissions are insufficient.
	 * 
	 * @param request               The incoming HttpServletRequest.
	 * @param response              The HttpServletResponse to modify.
	 * @param accessDeniedException The exception indicating the authorization failure.
